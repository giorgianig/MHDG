...
CONTAINS
  ...
  SUBROUTINE initialize_magnetic_field
    ...
    ALLOCATE (phys%safety_factor(nnodes))
  ...
  SUBROUTINE load_magnetic_field_grid()
    ...
    integer                           :: i, j, ierr, ip, jp, ind, k, ipsi
    ...
    CALL HDF5_array1D_reading(file_id, q, 'q')
...
#ENDIF 
  ...
  SUBROUTINE loadJtorMap()
    ...
    elseif (switch%testcase>=80 .and. switch%testcase<90) then
        ! ITER case
    		if(switch%ME .eq. .FALSE.) then !if not a moving equilibrium simulation
    	    !fname = 'ITER_2008_MagField.h5'
    	    fname = 'ITER_135011_Jtor_1600.h5'
			  else
  ...
  SUBROUTINE SetPuff()
    ...
    IF (switch%testcase .ge. 80 .and. switch%testcase .le. 89) THEN
        ...
        nlit = phys%n_li(1) + (phys%puff_slope/simpar%refval_density - phys%n_li(1))/(15 - 0.35)*(phys%I_p - 0.35)
        ...
        ELSE
           ...
           !IF (phys%n_li(time%it+1) < phys%n_li(time%it) .and. nli > nlit .and. nli < 1.03*nlit) THEN
!IF (phys%n_li(time%it+1) < phys%n_li(time%it) .and. nli > nlit .and. abs(nslope) > (1.5 + (10*(1 - nli/nlit))**2)) THEN
              !phys%puff_exp(time%it+1) = phys%puff_exp(time%it) + 50.*simpar%refval_density*max(1.03*nlit - nli,0.)
!phys%puff_exp(time%it+1) = max(phys%puff_exp(time%it) + 2.*simpar%refval_density*(abs(nslope) - 1.), 0.)
!IF (phys%puff_exp(time%it+1) - phys%puff_exp(time%it) == phys%puff_exp(time%it + 1)) phys%puff_exp(time%it + 1) = 1.e20
!ELSE
!phys%puff_exp(time%it+1) = max(phys%puff_exp(time%it) + sign(1.,1. - nli/nlit)*60.*simpar%refval_density*abs(nlit - nli), 0.) 
!IF (nli < nlit .and. phys%puff_exp(time%it+1) - phys%puff_exp(time%it) == phys%puff_exp(time%it + 1)) phys%puff_exp(time%it + 1) = 1.e20
              !IF (nli < nlit .and. nslope < 1) phys%puff_exp(time%it + 1) = phys%puff_exp(time%it+1) + simpar%refval_density*(1. - nslope)
!IF (phys%I_p < 14.8) THEN
                 !nslope = (phys%n_li(time%it+1) - phys%n_li(time%it))/(phys%n_lit(time%it+1) - phys%n_lit(time%it))
                 !WRITE(6,*) 'nslope = ', nslope
!IF (nli < nlit .and. nslope > (1.5 + (10*(1 - nli/nlit))**2)) THEN
                    !WRITE(6,*) 'Inside nslope i
!phys%puff_exp(time%it+1) = max(phys%puff_exp(time%it) - 2.*simpar%refval_density*(nslope - 1.), 0.)
!END IF
!END IF
!END IF
           phys%puff_exp(time%it+1) = max(phys%puff_exp(time%it) + sign(1.,1. - nli/nlit)*60.*simpar%refval_density*abs(nlit - nli) + 15.*simpar%refval_density*(1 - nslope), 0.)
           phys%puff = phys%puff_exp(time%it+1)
       END IF
       IF (MPIvar%glob_id .eq. 0) THEN
