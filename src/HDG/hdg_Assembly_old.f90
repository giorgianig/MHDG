...
!write(6,*) "MPIvar%itor: ",MPIvar%itor
!write(6,*) "nnz: ",nnz
!write(6,*) "linew: ",linew
!write(6,*) "shift: ",shift
!call mpi_barrier(mpi_comm_world,ierr)
     ...
     DO iel = 1,Nel
        ...
        IF (MPIvar%ntor.gt.1) THEN
           IF (MPIvar%itor==MPIvar%ntor) THEN
              ...
              IF (itor == Ntorloc-1) THEN
                 delta(Nf+2) = 1+(Easind(iel)-1)*blkp
              ENDIF     
              IF (itor == Ntorloc) THEN
                 delta(1) = 1+(Easind(iel)-1)*blkp
              ENDIF     
              
           ELSE
              delta =-(Nel-Nghoste)*blkp+ 1+(itor-1)*( (Nel-Nghoste)*blkp+(Nfaces-Ndirf-Nghostf)*blkt)+&
              &(/ (Easind(iel)-1)*blkp,(Nel-Nghoste)*blkp+(Fasind(Fe)-1)*blkt,(Nel-Nghoste)*blkp+(Nfaces-Ndirf-Nghostf)*blkt+(Easind(iel)-1)*blkp /)           
           ENDIF
        ELSE
           delta =1+(itor-1)*( (Nel-Nghoste)*blkp+(Nfaces-Ndirf-Nghostf)*blkt)+&
...
#endif						          
#ifdef PARALL							
				          IF (MPIvar%itor==numer%ntor .and.   itor==Ntorloc) THEN
				             Fig = iel                     ! Global numbering of the face
				          ELSE
...
#ifdef PARALL											
								   IF (Mesh%ghostElems(iel).eq.1)	CYCLE
								   IF ((MPIvar%ntor.gt.1 ) .and. (itor==ntorloc) ) CYCLE 
#endif						          
...
#ifdef PARALL							
				          IF (MPIvar%itor==numer%ntor .and.   itor==Ntorloc-1) THEN
				             Fig = iel                     ! Global numbering of the face
				          ELSE
  ...
  CONTAINS
     ...
     IF (MPIvar%itor.eq.MPIvar%ntor) THEN
        DO iel = 1,Nel      
           ...
           Fe = Mesh%F(iel,:)
	          ...
	          END DO			    
	          IF (Fig.gt.1) THEN        
              shift(Fig) = shift(Fig-1) + linew(Fig-1)*blkp 
           ENDIF
        END DO                 
     END IF 
     END IF
     ...
     DO itor = 1,Ntorloc
#ifdef PARALL
           IF ( (MPIvar%ntor.gt.1)    .and. (itor==Ntorloc)  )  CYCLE ! ghost toroidal element			        
#endif        
        ! ************************************************
...
#ifdef PARALL
           IF ( (MPIvar%ntor.gt.1)    .and. (itor==1)  )  THEN ! toroidal ghost face											
								      IF (Fig.gt.1) THEN
						        shift(Fig) = shift(Fig-1)
						        linew(Fig) = linew(Fig-1)
						        END IF	
...
!						        shift(Fig) = shift(Fig-1) + linew(Fig-1)*blkp
     ...
     ELSE
        ...
        ELSE
        ! ****************************************
        ! Case more than 1 toroidal divisions
        ! ****************************************       
           IF (itor==1) THEN  
     ...
     ! ****************************************       
        ...
        !             [O x x x O] O x x x O
           ...
           END IF
           IF (itor==Ntorloc) THEN
        !!TODO verificare che qui sto entrando!!! 
        !! Questo caso sembra inutile, ho già verificato che itor ~= Ntorloc
					         pos_start(Nf+2) = 0
					      END IF             
        END IF
     END IF
...
!					posg(1:Nf+2)
     ...
     ! the coefficients in the cols and vals vectors							
       ...
       IF (MPIvar%ntor.eq.1) THEN
							   IF (itor==Ntorloc .and. Ntorloc>1) THEN
							      pos_start = pos_start+2*blkp
							      pos_start(Nf+2)=0
										   IF (iel>ieln) THEN
													   pos_start(Nf+2) = pos_start(Nf+2)+blkp
										   END IF							   
							   ELSE IF (Ntorloc==1) THEN
										   IF (iel>ieln) THEN
													   pos_start(Nf+2) = blkp
										   ELSE
							         pos_start(Nf+2) = 0
