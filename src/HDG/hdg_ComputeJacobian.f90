...
!TOR3D
  ...
  DO iel = 1,N2D
    ...
    ! Safety factor of the nodes of the element 
    qsfel = phys%safety_factor(Mesh%T(iel,:))
    ...
    DO ifa=1,refElPol%Nfaces
      ...
      ! Safety factor of the nodes of the face
      qsffl = phys%safety_factor(Mesh%T(iel,refElPol%face_nodes(ifa,:)))
      ...
      if (iface.le.Mesh%Nintfaces) then
        CALL elemental_matrices_faces_int(iel,ifa,Xfl,Bfl,psifl,qsffl,qef,uef,uf,diff_nn_Fac_el,v_nn_Fac_el,tau_save_el,xy_g_save_el)		 
      else
        if (Mesh%periodic_faces(iface-Mesh%Nintfaces).eq.0) then
          CALL elemental_matrices_faces_ext(iel,ifa,isdir,Xfl,Bfl,psifl,qsffl,qef,uef,uf,diff_nn_Fac_el,v_nn_Fac_el,tau_save_el,xy_g_save_el)
        else
          ! periodic face
          CALL elemental_matrices_faces_int(iel,ifa,Xfl,Bfl,psifl,qsffl,qef,uef,uf,diff_nn_Fac_el,v_nn_Fac_el,tau_save_el,xy_g_save_el)
        endif
...
CONTAINS
  ...
  SUBROUTINE elemental_matrices_volume(iel,Xel,Bel,fluxel,psiel,qsfel,qe,ue,u0e,Jtorel,El_n,El_nn,Sel_ICRH,Sel_ECRH,diff_nn_Vol_el,v_nn_Vol_el,Xg_el)
    ...
    real*8                        :: iJ11(Ng2d),iJ12(Ng2d)
    real*8                        :: iJ21(Ng2d),iJ22(Ng2d)
    real*8                        :: fluxg(Ng2d),max_flux2D,min_flux2D,Psig(Ng2d),qsfg(Ng2d)
    integer*4,dimension(Npel)     :: ind_ass,ind_asq
    real*8                        :: ktis(time%tis + 1)
    ...
    ! Safety factor at Gauss points
    qsfg = matmul(refElPol%N2D,qsfel)
...
!#endif
    ...
    DO g = 1,NGauss
      ...
      CALL assemblyVolumeContribution(Auq,Auu,rhs,b(g,:),Psig(g),qsfg(g),divbg,driftg,Bmod(g),force(g,:),&
        &ktis,diff_iso_vol(:,:,g),diff_ani_vol(:,:,g),Ni,NNi,Nxyzg,NNxy,NxyzNi,NNbb,upg(g,:),&
  ...
  !***************************************************
  SUBROUTINE elemental_matrices_faces_int(iel,ifa,Xfl,Bfl,psifl,qsffl,qef,uef,uf,diff_nn_Fac_el,v_nn_Fac_el,tau_save_el,xy_g_save_el)
    integer,intent(IN)        :: iel,ifa
    real*8,intent(IN)         :: Xfl(:,:)
    real*8,intent(IN)         :: Bfl(:,:),psifl(:),qsffl(:)
    real*8,intent(IN)         :: qef(:,:)
    real*8,intent(IN)         :: uef(:,:),uf(:,:)
    ...
    real*8                    :: Bmod_nod(Npfl),b_nod(Npfl,3),b(Ng1d,3),Bmod(Ng1d),Psig(Ng1d),qsfg(Ng1d)
    ...
    DO g = 1,NGauss
      ...
      ! Assembly local contributions
      CALL assemblyIntFacesContribution(iel,ind_asf,ind_ash,ind_ff,ind_fe,ind_fg,b(g,:),Bmod(g),Psig(g),qsfg(g),&
...
!stop
  ...
  !***************************************************
  SUBROUTINE elemental_matrices_faces_ext(iel,ifa,isdir,Xfl,Bfl,psifl,qsffl,qef,uef,uf,diff_nn_Fac_el,v_nn_Fac_el,tau_save_el,xy_g_save_el)
    integer,intent(IN)        :: iel,ifa
    real*8,intent(IN)         :: Xfl(:,:)
    real*8,intent(IN)         :: Bfl(:,:),psifl(:),qsffl(:)
    logical,intent(IN)        :: isdir
    ...
    DO g = 1,NGauss
      ...
      ELSE
        CALL assemblyExtFacesContribution(iel,isdir,ind_asf,ind_ash,ind_ff,ind_fe,ind_fg,b(g,:),Bmod(g),Psig(g),qsfg(g),&
          n_g,diff_iso_fac(:,:,g),diff_ani_fac(:,:,g),NNif,Nif,Nfbn,ufg(g,:),upgf(g,:),qfg(g,:),tau)
      ENDIF
#else
      CALL assemblyExtFacesContribution(iel,isdir,ind_asf,ind_ash,ind_ff,ind_fe,ind_fg,b(g,:),Bmod(g),Psig(g),qsfg(g),&
...
!TOR3D
  ...
  !
  !********************************************************************
  SUBROUTINE assemblyVolumeContribution(Auq,Auu,rhs,b3,psi,qsf,divb,drift,Bmod,f,&
      &ktis,diffiso,diffani,Ni,NNi,Nxyzg,NNxy,NxyzNi,NNbb,upe,ue,qe,u0e,xy,Jtor,Vnng)
    real*8,intent(inout)      :: Auq(:,:,:),Auu(:,:,:),rhs(:,:)
    real*8,intent(IN)         :: b3(:),psi,qsf,divb,drift(:),f(:),ktis(:),Bmod
    ...
    real*8                    :: dPi_dU(Neq),Sohmic,dSohmic_dU(Neq) ! Ohmic heating
...
#ifndef NEUTRALP
    ! Linarization Dnn
    call compute_dDnn_dU(ue,dDnn_dU)
#endif
    ...
    !call compute_dsigmaviz_dne(ue,dsigmaviz_dne)
    call compute_dsigmavrec_dU(ue,dsigmavrec_dU)
    !call compute_dsigmavrec_dne(ue,dsigmavrec_dne)
#endif
...
!NEUTRAL
    ...
    ! Source to limit low temperature values
    ! Ion Temperature
    !IF (upe(7) .le. 6.e-4) THEN
    !   i = 3
    !   DO j = 1,Neq
    !      z = i + (j-1)*Neq
    !      Auu(:,:,z) = Auu(:,:,z) + 5.*2.7472e-6*dPi_dU(j)*NNi
    !      IF (j == 1) THEN
    !         Auu(:,:,z) = Auu(:,:,z) - 5.*2.7472e-6*3./2.*phys%Mref*6.e-4*NNi
    !      END IF
    !   END DO
    !END IF
    !Electron temperature
    !IF (upe(8) .le. 6.e-4) THEN
       !WRITE(6,*) 'Te = ', upe(8)
    !   i = 4
    !   z = i + (i-1)*Neq
    !   Auu(:,:,z) = Auu(:,:,z) + NNi!*5*2.7472e-6
    !   j = 1
    !   z = i + (j-1)*Neq
    !   Auu(:,:,z) = Auu(:,:,z) - 3./2.*phys%Mref*6.e-4*NNi!*5*2.7472e-6
    !END IF
      ...
      IF (switch%driftdia .and. i.ne.4) THEN
        ...
        ELSEIF (i == 4) THEN
          ...
          ELSEIF(i == 5) THEN
             ...
             END DO
             DO k = 1, Ndim
                rhs(:,i) = rhs(:,i)+dot_product(dDnn_dU,ue)*Qpr(k,i)*Nxyzg(:,k)
             END DO
#else		  
          ELSEIF (i == 5) THEN
             DO j = 1,5
	        DO k = 1,Ndim
	           z = i+(k-1)*Neq+(j-1)*Neq*Ndim
                   Auu(:,:,i+(j-1)*Neq) = Auu(:,:,i+(j-1)*Neq) + (Dpn*Taupn(k,j) + dDpn_dU(j)*gmpn(k))*GammaLim*NxyzNi(:,:,k) 
                   Auq(:,:,z) = Auq(:,:,z) + Dpn*Vpn(j)*GammaLim*NxyzNi(:,:,k)
                END DO
             END DO
             DO k = 1,Ndim
                rhs(:,i) = rhs(:,i) + dot_product(dDpn_dU,ue)*gmpn(k)*GammaLim*Nxyzg(:,k)
             END DO
#endif
#ifdef NEUTRALGAMMA
         ELSEIF (i == 6) THEN
            DO j = 1,Neq
               z = i+(j-1)*Neq
               ! Curvature term Neutral parallel momentum
               Auu(:,:,z) = Auu(:,:,z) - GGn(i,j)*NNi
               ! Viscosity Neutral parallel momentum
               DO k = 1,Ndim
                   z = i+(k-1)*Neq+(j-1)*Neq*Ndim
    ...
    SUBROUTINE assemblyIntFacesContribution(iel,ind_asf,ind_ash,ind_ff,ind_fe,&
        &ind_fg,b3,Bmod,psi,qsf,n,diffiso,diffani,NNif,Nif,Nfbn,uf,upf,qf,tau,Vnng,ifa)
      integer*4,intent(IN)      :: iel,ind_asf(:),ind_ash(:),ind_ff(:),ind_fe(:),ind_fg(:)
      real*8,intent(IN)         :: b3(:),n(:),Bmod,psi,qsf
      real*8,intent(IN)         :: diffiso(:,:),diffani(:,:)
...
#ifndef NEUTRALP
      ! Linearization Dnn
      call compute_dDnn_dU(uf,dDnn_dU)
#else
    ! Compute Vpn(U^(k-1))
    ...
    ! Compute flux limiter
    CALL computeGammaLim(uf,Qpr,Vpn,GammaLim)
#endif
#ifdef NEUTRALGAMMA
    ! Viscosity term
      ...
      DO i = 1,Neq
       ...
       ELSEIF (i == 5) THEN
          DO j = 1,Neq
             ...
             DO k = 1,Ndim
                ind_kf = k + (j - 1)*Ndim + ind_ash
                kmult = (Dpn*Taupn(k,j) + dDpn_dU(j)*gmpn(k))*GammaLim*n(k)*NNif 
                elMat%Aul(ind_fe(ind_if),ind_ff(ind_jf),iel) = elMat%Aul(ind_fe(ind_if),ind_ff(ind_jf),iel) - kmult
                elMat%All(ind_ff(ind_if),ind_ff(ind_jf),iel) = elMat%All(ind_ff(ind_if),ind_ff(ind_jf),iel) - kmult
                kmult = Dpn*Vpn(j)*GammaLim*n(k)*NNif
                elMat%Auq(ind_fe(ind_if),ind_fg(ind_kf),iel) = elMat%Auq(ind_fe(ind_if),ind_fg(ind_kf),iel) - kmult
                elMat%Alq(ind_ff(ind_if),ind_fg(ind_kf),iel) = elMat%Alq(ind_ff(ind_if),ind_fg(ind_kf),iel) - kmult
             END DO
          END DO
          kmultf = dot_product(dDpn_dU,uf)*(gmpn(1)*n(1) + gmpn(2)*n(2))*GammaLim*Nif              
          elMat%S(ind_fe(ind_if),iel) = elMat%S(ind_fe(ind_if),iel) - kmultf
          elMat%fh(ind_ff(ind_if),iel) = elMat%fh(ind_ff(ind_if),iel) - kmultf 
#endif
#ifdef NEUTRALGAMMA
...
!      if (Ti .le. 6.e-4 .or. Te .le. 6.e-4) then 
         ! Plasma density
         !Sn(1,1)   = ad*(dnrec_dU(1)*sigmavrec + nrec*dsigmavrec_dU(1))
         !Sn(1,4)   = ad*nrec*dsigmavrec_dU(4)
         !Sn(1,5)   = 0.
         !Sn0(1)    = -ad*nrec*sigmavrec
         ! Plasma momentum
         !Sn(2,:)   = 0.
         !Sn0(2)    = 0.
         ! Ion Energy
...
!         Sn0(4) = 0.
         ! Neutral density
         !Sn(5,1)   = ad*dniz_dU(1)*sigmaviz
         !Sn(5,5)   = ad*(dniz_dU(Neq)*sigmaviz)
         !Sn(5,1)   = ad*(niz*dsigmaviz_dU(1))
         !Sn(5,4)   = ad*(niz*dsigmaviz_dU(4))
         !Sn0(5)    = -ad*(niz*sigmaviz)
!      endif
!#endif
