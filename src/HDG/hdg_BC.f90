...
CONTAINS
  ...
  real*8                    :: Bfl(refElPol%Nfacenodes,3),psifl(refElPol%Nfacenodes),qsffl(refElPol%Nfacenodes)
  real*8                    :: Bmod_nod(refElPol%Nfacenodes),b_nod(refElPol%Nfacenodes,3)
  real*8                    :: xyg(refElPol%Ngauss1d,2),xyder(refElPol%Ngauss1d,2)
  real*8                    :: Bmod(refElPol%Ngauss1d),b(refElPol%Ngauss1d,3),psig(refElPol%Ngauss1d),qsfg(refElPol%Ngauss1d)
  ...
  DO ifa = 1,Mesh%Nextfaces
    ...
    qsffl = phys%safety_factor(Mesh%T(iel,nod))
    ...
    DO g = 1,Ng1d
      ...
      if (numer%bohmtypebc.eq.0) then
        ...
        CALL assembly_bohm_bc(iel,ind_asf,ind_ash,ind_ff,ind_fe,ind_fg,NiNi,Ni,qfg(g,:),&
          &ufg(g,:),upg(g,:),b(g,1:2),psig(g),qsfg(g),n_g,tau_stab,setval,delta,diff_iso_fac(:,:,g),diff_ani_fac(:,:,g),ntang,Vnng)
#else
        CALL assembly_bohm_bc(iel,ind_asf,ind_ash,ind_ff,ind_fe,ind_fg,NiNi,Ni,qfg(g,:),&
        ...
        ! Summing conribution from each gauss point on the face
...
!    elMat%fh(ind_ff(indi),iel) = elMat%fh(ind_ff(indi),iel) - source_dens_coeff*Ni 
  ...
  !*********************************
#ifndef SAVEFLUX
  SUBROUTINE assembly_bohm_bc(iel,ind_asf,ind_ash,ind_ff,ind_fe,ind_fg,NiNi,Ni,qfg,ufg,upfg,bg,psig,qsfg,ng,tau,setval,delta,diffiso,diffani,ntang,Vnng)
#else
  SUBROUTINE assembly_bohm_bc(iel,ind_asf,ind_ash,ind_ff,ind_fe,ind_fg,NiNi,Ni,qfg,ufg,upfg,bg,psig,ng,tau,setval,delta,diffiso,diffani,dline,ntang,Vnng,flgflux_puff,flgflux_parallel,flgflux_perpendicular,flgflux_neutral)
    ...
    real*8           :: NiNi(:,:),Ni(:),ufg(:),upfg(:),bg(:),psig,qsfg,ng(:),tau(:,:),setval,delta
    real*8           :: diffiso(:,:),diffani(:,:)
    logical          :: ntang
    real*8           :: qfg(:),Vnng(:)
    ...
    ! Compute Vu(U^(k-1))
    call computeVu(ufg,Vu)
    call compute_dVu_dU(ufg,dVu_dU)
    Tauu = matmul(Qpr,dVu_dU)
    ...
    IF (numer%stab < 6) THEN
      DO i = 1,Neqstab
         ...
         !      DO j = 1,Neqstab
         !         indj = j + ind_asf
         !         elMat%All(ind_ff(ind),ind_ff(indj),iel) = elMat%All(ind_ff(ind),ind_ff(indj),iel) - tau(i,i)*dPi_dU(j)*NiNi
         !      END DO
         !   ELSE
         !      elMat%All(ind_ff(ind),ind_ff(ind),iel) = elMat%All(ind_ff(ind),ind_ff(ind),iel) - tau(i,i)*NiNi
         !   END IF
         ...
         !   END IF
         !ELSE IF (i == 4) THEN
         !   IF (upfg(8) .le. 6.e-10) THEN
         !      elMat%Alu(ind_ff(ind),ind_fe(ind - 3),iel) = elMat%Alu(ind_ff(ind),ind_fe(ind - 3),iel) + tau(i,i)*3./2*phys%Mref*6.e-4*NiNi
         !   ELSE
         !      elMat%Alu(ind_ff(ind),ind_fe(ind),iel) = elMat%Alu(ind_ff(ind),ind_fe(ind),iel) + tau(i,i)*NiNi
         !   END IF
#ifdef NEUTRALGAMMA
         ELSE IF (i == 6) THEN
            elMat%Alu(ind_ff(ind),ind_fe(ind - 1),iel) = elMat%Alu(ind_ff(ind),ind_fe(ind - 1),iel) + tau(i,i)*delta*(-upfg(2))*NiNi
            !elMat%Alu(ind_ff(ind),ind_fe(ind - 1),iel) = elMat%Alu(ind_ff(ind),ind_fe(ind - 1),iel) + tau(i,i)*(1 - delta)*(-upfg(2))*NiNi
#endif
         ELSE
            elMat%Alu(ind_ff(ind),ind_fe(ind),iel) = elMat%Alu(ind_ff(ind),ind_fe(ind),iel) + tau(i,i)*NiNi
         END IF
...
!    ! Perpendicular diffusion
    ...
    ! **********************************************
    ! Perpendicular diffusion (equation by equation)
    ! **********************************************
    ! Equation 1: grad_perp n = 0.
!    k = 1
!    DO idm = 1,Ndim
!       indi = ind_asf + k
!       indj = ind_ash + idm + (k - 1)*Ndim
!       elMat%Alq(ind_ff(indi),ind_fG(indj),iel)=elMat%Alq(ind_ff(indi),ind_fG(indj),iel)-&
!            &NiNi*(ng(idm) - bn*bg(idm))
!    END DO
!    ! Equation 2: grad_perp u = 0.
!    k = 2
!    DO idm = 1,Ndim
!       indi = ind_asf + k
!       DO j = 1,Neq
!          indj = ind_ash + idm + (j - 1)*Ndim
!          elMat%Alq(ind_ff(indi),ind_fG(indj),iel) = elMat%Alq(ind_ff(indi),ind_fG(indj),iel) + Vu(j)*NiNi*(ng(idm) - bn*bg(idm))
!          indj = ind_asf + j
!          elMat%All(ind_ff(indi),ind_ff(indj),iel)  = elMat%All(ind_ff(indi),ind_ff(indj),iel) + Tauu(idm,j)*NiNi*(ng(idm) - bn*bg(idm))
!       END DO
!       elMat%fh(ind_ff(indi),iel) = elMat%fh(ind_ff(indi),iel) + dot_product(Tauu(idm,:),ufg)*Ni*(ng(idm) - bn*bg(idm))
!    END DO
!#IFDEF TEMPERATURE
!    ! Equation 3: grad_perp Ti = 0
!    k = 3
!    DO idm = 1,Ndim
!       indi = ind_asf + k
!       DO j = 1,Neq
!          indj = ind_ash + idm + (j - 1)*Ndim
!          elMat%Alq(ind_ff(indi),ind_fG(indj),iel) = elMat%Alq(ind_ff(indi),ind_fG(indj),iel) + Vveci(j)*NiNi*(ng(idm) - bn*bg(idm))
!          indj = ind_asf + j
!          elMat%All(ind_ff(indi),ind_ff(indj),iel)  = elMat%All(ind_ff(indi),ind_ff(indj),iel) + Taui(idm,j)*NiNi*(ng(idm) - bn*bg(idm))
!       END DO
!       elMat%fh(ind_ff(indi),iel) = elMat%fh(ind_ff(indi),iel) + dot_product(Taui(idm,:),ufg)*Ni*(ng(idm) - bn*bg(idm))
!    END DO
!    ! Equation 4: grad_perp Te = 0.
!    k = 4
!    DO idm = 1,Ndim
!       indi = ind_asf + k
!       DO j = 1,Neq
!          indj = ind_ash + idm + (j - 1)*Ndim
!          elMat%Alq(ind_ff(indi),ind_fG(indj),iel) = elMat%Alq(ind_ff(indi),ind_fG(indj),iel) + Vvece(j)*NiNi*(ng(idm) - bn*bg(idm))
!          indj = ind_asf + j
!          elMat%All(ind_ff(indi),ind_ff(indj),iel)  = elMat%All(ind_ff(indi),ind_ff(indj),iel) + Taue(idm,j)*NiNi*(ng(idm) - bn*bg(idm))
!       END DO
!       elMat%fh(ind_ff(indi),iel) = elMat%fh(ind_ff(indi),iel) + dot_product(Taue(idm,:),ufg)*Ni*(ng(idm) - bn*bg(idm))
!    END DO
!#ENDIF
    ...
    ! Parallel plasma flux contribution normal to the wall 
    flgflux_parallel = recycling_coeff*ufg(2)*bn
    ! Dimensionalizing and multiplying by the surface under this gauss point 
    flgflux_parallel = flgflux_parallel*2.*PI*dline*simpar%refval_density*simpar%refval_speed*simpar%refval_length**2
...
!#endif
    ...
    ! Neutral parallel momentum gradient normal to the wall = 0.
    k = 6
    indi = k+ind_asf
    DO idm = 1,Ndim
       indj = ind_ash+idm+(k-1)*Ndim
