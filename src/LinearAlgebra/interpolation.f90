...
contains
  ...
  real function interp1d(x_len, x_array, f, x, delta)
    ...
    ! values specified by x_array
    ! This function linear extrapolate points out of the grid x 
    ! Reference: http://en.wikipedia.org/wiki/Linear_interpolation
    implicit none
    integer, intent(in) :: x_len
    real, dimension(x_len), intent(in) :: x_array
    real, dimension(x_len), intent(in) :: f
    real, intent(in) :: x
    real, intent(in), optional :: delta
    !f2py depend(x_len) x_array, f
    !f2py depend(y_len) y_array, f
    ...
    IF (i == x_len) THEN ! Linear extrapolate
       x1 = x_array(i - 1)
       x2 = x_array(i)
       denom = (x2 - x1)
       interp1d = (f(i - 1)*(x2 - x) + f(i)*(x - x1))/denom
    ELSE
       x1 = x_array(i)
       x2 = x_array(i + 1)
       denom = (x2 - x1)
       interp1d = (f(i)*(x2 - x) + f(i + 1)*(x - x1))/denom
    END IF
