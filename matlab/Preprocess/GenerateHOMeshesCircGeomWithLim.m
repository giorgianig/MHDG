...
ne1 = 4;      % number of layers in the closed-line zone
ne2 = 4;    % number of layers in the open-line zone (SOL)
...
sref = 2;      % refinement on the separatrix: the number of final layers corresponds to the original one multiplied by this
factors = 2; % refinement on the separatrix: intensity
...
for ic=nsiz
    ...
    for ip =npol
        ...
        % Non refined part
        [X,T]     = CreaMalla(elemType,NNodesLin,xs,1-xs,0    ,s-ys1,nx-2*nlref,ne1-nsref);
        [X3,T3] = CreaMalla(elemType,NNodesLin,xs,1-xs,s-ys1,s    ,nx-2*nlref,nyref);
        q = (s-X3(:,2))/ys1;
        ...
        [X2,T2] = CreaMalla(elemType,NNodesLin,0,xs,0     ,s-ys1,nxref,ne1-nsref);
        [X3,T3] = CreaMalla(elemType,NNodesLin,0,xs,s-ys1,s     ,nxref,nyref);
        q = (s-X3(:,2))/ys1;
        r = (exp(factors*q)-1)/(exp(factors)-1);
        X3(:,2) = s-r*ys1;
        ...
        [X2,T2] = CreaMalla(elemType,NNodesLin,1-xs,1,0      ,s-ys1,nxref,ne1-nsref);
        [X3,T3] = CreaMalla(elemType,NNodesLin,1-xs,1,s-ys1,s       ,nxref,nyref);
        q = (s-X3(:,2))/ys1;
        r = (exp(factors*q)-1)/(exp(factors)-1);
        X3(:,2) = s-r*ys1;
        [X2,T2] = sawMeshes(X2,T2,X3,T3,s-ys1,2);
        ...
        [X1,T1] = CreaMalla(elemType,NNodesLin,xs,1-xs,s+ys2,1      ,nx-2*nlref,ne2-nsref);
        ...
        % Part refined on the limiter sx
        [X2,T2] = CreaMalla(elemType,NNodesLin,0,xs,s+ys2,1      ,nxref,ne2-nsref);
        ...
        % Part refined on the limiter dx
        [X2,T2] = CreaMalla(elemType,NNodesLin,1-xs,1,s+ys2,1,nxref,ne2-nsref);
        ...
        if ip>1
            ...
            [X, T, Tb_UP, Tb_DOWN, Tb_LEFT, Tb_RIGHT, ...
                ...
                (X, T, Tb_UP, Tb_DOWN, Tb_LEFT, Tb_RIGHT,...
                elemInfo, elementFaceInfo, ip,elemType);
        end
