function [x, flag, total_iters, error] = gmres_r(A,M, b, params, x0)
...
%% Modification by HCE, 15 Oct. 2004: adapt for right preconditioning
    v(:,k+1) = A*(M\v(:,k));%feval(afun, feval(mfun,v(:,k),aparams,mparams), aparams);
    normav=norm(v(:,k+1));
%
...
x = x0 + M\(v(1:n,1:k)*y);%feval(mfun, v(1:n,1:k)*y, aparams,mparams);
