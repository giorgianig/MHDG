...
plotDiscontinuousSolution(X,T,sqrt(u(1:2:end-1).^2 + u(2:2:end).^2),refEl)
plotMesh_HOElement(X',T)
hold on
...
motherMesh = load('../../Meshes/NACA0012/FAR_10_ComparisonHO/mother_mesh_P8.mat');
...
for ielem = 1:numel(elements)
    ...
    for iline = 1:nx
        ...
        quiver(xy(:,1),xy(:,2),vel(:,1)/scale,vel(:,2)/scale,0,'k')
...
for ielem = 1:nelem
    ...
    for ilev = 2:2*nlev
        ...
        for i = 1:size(x_conn,1)
            ...
            if all(unique(ceil(x_act*coef)) == unique(ceil(x_conn(i,:)*coef)))
