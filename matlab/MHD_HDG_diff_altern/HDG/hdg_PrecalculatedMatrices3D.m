...
global kT neq diff_n diff_u Magnetic testcase axisym driftvel refElTor
...
%  display(['force: ', num2str(force(1)),'  ',num2str(force(2))])       
        ...
        if (testcase.n >= 50 && testcase.n<60)
            ...
            if driftvel
                [b,divb,dvx,dvy] = defineMagneticField([xg,yg]);
                dv = [dvx,dvy];
            else
                [b,divb] = defineMagneticField3d([xg,yg],tg);
            end
        end
        ...
        if driftvel
            Dve = Dve + Ni'*(dv(1)*Nx + dv(2)*Ny+ dv(3)*Nt)*dvolu;
        end
...
for iface = 1:(nf+2)
    ...
    u_ex = analyticalSolution3d(xyfg,thetafg);
     ...
     if (testcase.n >= 50 && testcase.n<60)
        ...
        b = defineMagneticField3d([xyfg(:,1),xyfg(:,2)],thetafg(:));
    end   
...
diffdiag = repmat(col(repmat([diff_n diff_u],Ndim,1)),Nv,1);
Diff = diag(diffdiag);
% Lf = Lf_transp'*Diff;
Lf = Lf*Diff;
Qf = Qf*Diff;
