  function [c,h]=tricontour_h(Mesh,z,nv,varargin)
...
%plot_vacuum_xiao(Mesh,psiInit(1:end-1),id_ax,id_bd,psiInit(id_bd)); % only plot the plasma domain 
% the h in lines 201 is the attibute of the iso-contour of nv of the input
% items. Xiao, 22/03/2017
%
...
for v=1:nlev                  % one contour level at a time
   ...
   while any(flag)	% while there are still unused edges -----------------
      ...
      if nargout<2                     % plot the subcontour
          if nlev==1
              if (nargin<6)
                  patch('XData',xtmp,'YData',ytmp,'CData',repmat(zc,k,1),...
                      'Parent',hax,'FaceColor','none','EdgeColor','m',...
                      'UserData',zc)
                  
              else
                  patch('XData',xtmp,'YData',ytmp,'CData',repmat(zc,k,1),...
                      'Parent',hax,'FaceColor','none','EdgeColor',varargin{1},...
                      'UserData',zc)
              end
          else
              patch('XData',xtmp,'YData',ytmp,'CData',repmat(zc,k,1),...
         ...
         h=[h;patch('XData',xtmp,'YData',ytmp,'CData',repmat(zc,k,1),...  %% change the linestyle of the boundary, Xiao, 22/03/2017
         'Parent',hax,'FaceColor','none','EdgeColor','r',...              %% change the linestyle of the boundary, Xiao, 22/03/2017
         'UserData',zc,'lineWidth',1,'linestyle','-')]; %#ok
         C=horzcat(C,[zc xtmp';k ytmp']); % contour label data
      end
   end % while any(flag) --------------------------------------------------
end % for v=1:nlev
