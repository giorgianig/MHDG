function [error eg domArea] = calculateL2ErrorTwoSolutionDifferentP_scalar(X1,T1,u1,refEl1,X2,T2,u2,refEl2)
...
ueg = n(:,:,1)*ue;
err_gauss = Ueg-ueg;
elemError = err_gauss(:)'*(dvolu.*err_gauss(:));
elemArea = sum(dvolu);
solNorm = Ueg(:)'*(dvolu.*Ueg(:));
