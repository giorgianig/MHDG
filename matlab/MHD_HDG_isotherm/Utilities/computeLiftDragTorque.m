function [L D T] = computeLiftDragTorque(X,T,infoFaces,L_grad,p,refElv,refElp,mu,theta)
...
for iFace = 1:numel(elements)
    ...
    b_vect = [-y_g(:,iFace) x_g(:,iFace)-1/4];
...
Lr = L*cos(theta) - D*sin(theta);
Dr = L*sin(theta) + D*cos(theta);
L = Lr; D = Dr;