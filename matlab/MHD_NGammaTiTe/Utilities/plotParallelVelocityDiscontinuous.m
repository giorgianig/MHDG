function plotParallelVelocityDiscontinuous(X,T,sol,referenceElement,nDegRef)
...
for ielem = 1:nOfElements
    ...
    uplot = shapeFunctions*sol(ind,:);
    pv = uplot(:,2)./uplot(:,1);

    % Plot interpolated solution in the element
    hold on
    patchHandle(ielem) = patch('Faces',elemTriRef,'Vertices',Xplot,'FaceVertexCData',pv,...
