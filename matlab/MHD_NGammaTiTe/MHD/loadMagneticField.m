...
Magnetic.div_nodes = interp2(magfieldwest.r2D,magfieldwest.z2D,div,reshape(X(T',1),size(T')),reshape(X(T',2),size(T')));


%% modification : interpolation via shape functions
Magnetic.bx = refEl.N*Magnetic.bxnodes;
Magnetic.by = refEl.N*Magnetic.bynodes;
Magnetic.div = refEl.N*Magnetic.div_nodes;
...
%         Magnetic.byfaces(:,Face) = interp2(magfieldwest.r2D,magfieldwest.z2D,by,xgauss,ygauss);
        Magnetic.bxfaces(:,Face) = refEl.N1d*Magnetic.bxnodes(refEl.faceNodes(ifac,:),iel);
        Magnetic.byfaces(:,Face) = refEl.N1d*Magnetic.bynodes(refEl.faceNodes(ifac,:),iel);
