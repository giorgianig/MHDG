function [Df,Ef,Hf,Lf,Qf,fH,TUhf,TQhf,Tfhf] = hdg_bc_3D_aux(Df,Ef,Hf,Lf,Qf,TUhf,TQhf,Tfhf,X,T,infoFaces,refEl,bn,           F,F_dir,sol_hat,solQ)
...
Cnx = zz;Cny=zz;Cnt=zz;
Qbx = zz;Qby=zz;Qbt=zz;
...
%         NN  =  Nfg'*Nfg*dsurf(g);
 ...
 %% NEW****************************************************************      
    NNf =  Nfg'*dsurf(g)*dot(n,b);
    NN  =  Nfg'*Nfg*dsurf(g)*dot(n,b);
    NN0 = Nfg'*Nfg*dsurf(g);
%% NEW****************************************************************    
...
%         fH = fH + [delta_rho*limitRhoMin,0,-fan(g,3),-fan(g,4)]*NNf;
        ...
        Cny = Cny + NN0*n(2);    
        Cnt = Cnt + NN0*n(3);
        Qbx = Qbx + b(1)*Nfg'*(b(1)*Nfg*n(1)+b(2)*Nfg*n(2)+b(3)*Nfg*n(3))*dsurf(g);
        Qby = Qby + b(2)*Nfg'*(b(1)*Nfg*n(1)+b(2)*Nfg*n(2)+b(3)*Nfg*n(3))*dsurf(g);            
        Qbt = Qbt + b(3)*Nfg'*(b(1)*Nfg*n(1)+b(2)*Nfg*n(2)+b(3)*Nfg*n(3))*dsurf(g);
...
%% NEW*********************************************
Lf(ind_ff,ind_fq) =  transpose( expandMatrixB(Cnx',Cny',Cnt') );
Qf(ind_ff,ind_fq) =  transpose( expandMatrixB(Qbx',Qby',Qbt') );
if iota
    diffdiag = repmat(col(repmat([diff_n diff_u diff_ei diff_ee],Ndim,1)),nv,1);
    Diff = diag(diffdiag);
    Lf(ind_ff,ind_fq) = Lf(ind_ff,ind_fq)*Diff;
    Qf(ind_ff,ind_fq) = Qf(ind_ff,ind_fq)*Diff;
end
%% NEW*********************************************
