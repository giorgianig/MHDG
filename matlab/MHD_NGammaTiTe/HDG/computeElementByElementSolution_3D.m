function u = computeElementByElementSolution_3D(ntor,lambda,UU,U0,T,F,Fdir,refElPol,refElTor)
...
for itor = 1:ntor
    for iel = 1:N2d
        ...
        ind_ele = (iElem-1)*Npel*neq + (1:Npel*neq);
        Fe = F(iel,:);
        ...
        u(ind_ele) = UU(:,:,iElem)*lambda(ind_fac)+U0(:,iElem);
    end
end
