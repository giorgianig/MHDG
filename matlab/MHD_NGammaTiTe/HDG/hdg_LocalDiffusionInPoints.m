...
Lf_transp =  zeros(neq*nd*Nv,neq*nf*nv);
...
eps = computeLocalDiffusionInPoints(Xg*Mesh.lscale);
...
for g = 1:ngauss
    ...
    Px = Nx_g'*Niv_g*dvolu;
    Py = Ny_g'*Niv_g*dvolu;
    P = P + transpose(expandMatrixB(Px',Py',eps(g),eps(g),eps(g),eps(g)));
end
...
for iface = 1:nf
    ...
    yf = ye(nodesv);
    xyfg = N1dv*[xf yf];
    eps = computeLocalDiffusionInPoints(xyfg*Mesh.lscale);
    
    % initialize local matrices
    C_loc = zeros(neq*nv*nd,neq*nv);
    ...
    for g = 1:ngauss_f
        ...
        C_loc = C_loc + expandMatrixB(Cnx,Cny,eps(g),eps(g),eps(g),eps(g));
    end
...
function res = expandMatrixB(Bx,By,eps_n,eps_u,eps_ei,eps_ee)
% expand matrix B
%   [ Bx  0  0 
%     By  0  0 
%     0  Bx  0 
...
res(:,:,[1 2 11 12 21 22 31 32]) = cat(3,eps_n*Bx,eps_n*By,eps_u*Bx,eps_u*By,eps_ei*Bx,eps_ei*By,eps_ee*Bx,eps_ee*By);
res = permute(res, [3 1 4 2]);
