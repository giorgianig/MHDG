...
meshName  = 'CircLimAlign_Quads_Nel384_P3.mat';
% Number of divisions
ndiv = 48;
 divType = 'rot';
% divType = 'x';
...
for itry =1:100
    ...
    if any(abs(bal)>tol)
        for it = ndiv:-1:2
            theta(it) = theta(it) + bal(it)*0.1*(theta(it+1)-theta(it));
        end
        for ifor = 1:numel(theta_forb)
            k = abs(theta-theta_forb(ifor))<5e-3;
            theta(k) = theta(k)+5e-3;
...
disp(['Balancing achieved: ' num2str(100*bal') ' %'])
